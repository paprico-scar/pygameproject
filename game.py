...
def the_end(sc):  # окно конца игры кода вы пройдете все уровни
    ...
    cur = con.cursor()
    max_id = cur.execute('''SELECT MAX(id) FROM tb_score''').fetchone()[0] + 1
    add_list = [max_id, final_score]
    cur.execute('INSERT INTO tb_score VALUES(?,?);', add_list)
    con.commit()
    max_score = cur.execute('''SELECT MIN(score) FROM tb_score''').fetchone()
    font = pygame.font.Font('data/font/CustomFontTtf16H30.ttf', 25)
    sc.fill(BGCOLOR)
...
def show_go_screen(sc):  # окно конца игры когда герой погибает
    ...
    con = sqlite3.connect('data/db/game.db')
    cur = con.cursor()
    min_score = cur.execute('''SELECT MIN(score) FROM tb_score''').fetchone()
    ...
    if max_id[0] == 1:
        draw_text(f'Лучший счет: ', WHITE, WIDTH / 2, HEIGHT / 3, sc, font)
    else:
        draw_text(f'Лучший счет: {min_score[0]}', WHITE, WIDTH / 2, HEIGHT / 3, sc, font)
    draw_text(f"Счет: {final_score}", WHITE, WIDTH / 2, HEIGHT / 2, sc, font)
...
def lor_screen(sc):  # окно с историей игры
    ...
    draw_text('Давным-давно в одном королевстве правил король. Часто этот король', WHITE, WIDTH / 2, HEIGHT / 5, sc,
              font)
    draw_text('спал и не любил, когда его отрывали от погружения в иные миры. Для', WHITE, WIDTH / 2, HEIGHT / 5 + 20,
              sc,
              font)
